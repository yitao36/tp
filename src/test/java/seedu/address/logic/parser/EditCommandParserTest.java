...
public class EditCommandParserTest {
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + TAG_DESC_HUSBAND + EMAIL_DESC_AMY
                ...
                + EMERGENCY_NAME_DESC_BOB + EMERGENCY_PHONE_DESC_AMY;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_AMY).withAddress(VALID_ADDRESS_AMY)
                .withTags(VALID_TAG_HUSBAND, VALID_TAG_FRIEND).withEmergencyContact(VALID_NAME_BOB, VALID_PHONE_AMY)
                .build();
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        userInput = targetIndex.getOneBased() + EMERGENCY_NAME_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withEmergencyContact(VALID_NAME_AMY, null).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
        // emergency contact (phone only)
        userInput = targetIndex.getOneBased() + EMERGENCY_PHONE_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withEmergencyContact(null, VALID_PHONE_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
        // emergency contact (name + phone)
        userInput = targetIndex.getOneBased() + EMERGENCY_NAME_DESC_AMY + EMERGENCY_PHONE_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withEmergencyContact(VALID_NAME_AMY, VALID_PHONE_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
